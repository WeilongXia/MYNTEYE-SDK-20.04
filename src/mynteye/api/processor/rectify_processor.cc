...
void RectifyProcessor::stereoRectify(
    models::CameraPtr leftOdo, models::CameraPtr rightOdo, const CvMat *K1,
    const CvMat *K2, const CvMat *D1, const CvMat *D2, CvSize imageSize,
    const CvMat *matR, const CvMat *matT, CvMat *_R1, CvMat *_R2, CvMat *_P1,
    CvMat *_P2, double *T_mul_f, double *cx1_min_cx2, int flags, double alpha,
    CvSize newImgSize) {
  ...
  fc_new =
      (cvmGet(K1, idx ^ 1, idx ^ 1) + cvmGet(K2, idx ^ 1, idx ^ 1)) * ratio;
...
void RectifyProcessor::loadCameraMatrix(
...
std::shared_ptr<struct CameraROSMsgInfoPair> RectifyProcessor::stereoRectify(
  ...
  CvMat c_R = cv::Mat(cv_R), c_t = cv::Mat(cv_t);
  CvMat c_K1 = cv::Mat(K1), c_K2 = cv::Mat(K2), c_D1 = cv::Mat(D1),
        c_D2 = cv::Mat(D2);
  CvMat c_R1 = cv::Mat(R1), c_R2 = cv::Mat(R2), c_P1 = cv::Mat(P1),
        c_P2 = cv::Mat(P2);
  double T_mul_f;
  double cx1_min_cx2;
  stereoRectify(
      leftOdo, rightOdo, &c_K1, &c_K2, &c_D1, &c_D2,
      cvSize(image_size1.width, image_size1.height), &c_R, &c_t, &c_R1, &c_R2,
      &c_P1, &c_P2, &T_mul_f, &cx1_min_cx2);
